...
const matchesCollection = 'T20s'; // Replace with your collection name
...
// Get matches by series name
exports.getMatchesBySeriesName = async (req, res) => {
  try {
    await client.connect();
    const database = client.db(databaseName);
    const collection = database.collection(matchesCollection);

    // Access the series name from the route parameter
    const seriesName = req.params.seriesName;

    // Query to find matches where 'info.event.name' matches the series name
    const query = { "info.event.name": seriesName };

    const matches = await collection.find(query).toArray();

    if (matches.length === 0) {
      return res.status(404).json({ message: 'No matches found for this series' });
    }
